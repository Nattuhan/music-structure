...
クリックは検出拍をそのまま不規則に並べず、安定区間から推定した一定拍グリッドを使用します。通常再生中のステムへ周期的な強制シークや個別の速度補正は行いません。再生開始、明示的なシーク、ループ境界の変更時と読み込み待ちからの復帰時に全ステムを基準音声の同じ時刻へそろえます。自分で行う同期シークの`waiting`は、読み込み停止として再処理しません。クリック予約は曲中の時間差を再生速度で実時間へ換算し、速度変更・シーク・停止時に破棄します。スクラブ中は動画の位置だけを更新し、確定時に音声とステムを再開します。
