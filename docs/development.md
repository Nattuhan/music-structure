...
クリックに使う拍列は、音源と前後の拍から誤検知の根拠を確認できた区間を共通処理で補正します。本当のテンポ変化を一律に一定拍へ置き換えません。通常再生中のステムへ周期的な強制シークや個別の速度補正は行いません。再生開始、明示的なシーク、ループ境界の変更時と読み込み待ちからの復帰時に全ステムを基準音声の同じ時刻へそろえます。自分で行う同期シークの`waiting`は、読み込み停止として再処理しません。クリック予約は曲中の時間差を再生速度で実時間へ換算し、速度変更・シーク・停止時に破棄します。スクラブ中は動画の位置だけを更新し、確定時に音声とステムを再開します。
