# PracticeLab v1.2.4（未公開・ローカル検証版）

- BPM表示と補正操作をコンパクトに整理
- バックグラウンド・最小化中もクリックを予約するよう修正
- 描画停止中のクリック継続と停止操作の回帰テストを追加

ローカル検証用ビルド名: `PracticeLab-1.2.4-arm64.dmg`。GitHub Release・R2への公開は行っていません。

# PracticeLab v1.2.3（未公開・ローカル検証版）

- パート再生失敗時は元音源へ戻り、失敗パートの表示と再試行に対応
- 全パートミュート時は、音量変更や再生再開後も無音を維持
- 読み込み待ちからの復帰時に全パートを現在位置へ同期
- 再生速度に応じたクリック予約と、速度変更・シーク時の予約解除を修正
- パート再生成の失敗・キャンセル時に既存の再生用音源を保持
- 練習記録の表示を廃止し、「最後に開いた順」と並び順の保存に対応
- 再生状態・クリック予約を画面処理から分離し、単体・画面・再生成の回帰テストを追加

ローカル検証用ビルド名: `PracticeLab-1.2.3-arm64.dmg`。GitHub Release・R2への公開は行っていません。
